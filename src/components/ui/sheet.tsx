...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      'fixed inset-0 z-50 bg-cocoa/40 backdrop-blur-[2px]',
...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-background shadow-lift transition ease-elegant data-[state=closed]:duration-400 data-[state=open]:duration-400',
  {
    variants: {
      side: {
...
const SheetFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn('flex flex-col-reverse gap-2 p-6 sm:flex-row sm:justify-end sm:gap-3', className)}
    {...props}
  />
);
